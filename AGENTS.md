...
> **Project Origin**: A highly-concurrent, horizontally scalable agentic coding environment. This repository implements a Database-Oriented Operating System (DBOS) architecture.
...
The DBOS MCP architecture decouples heavy Node.js LLM execution environments from lightweight client UIs. By replacing in-memory Node.js loops with a stateless client-server model, multiple agents can safely execute concurrent tasks.
...
- **High-Performance Storage Recommended**: To ensure ultra-low-latency database reads/writes and embedding queries, all Docker containers, event streams, and PostgreSQL databases should reside natively on a high-speed SSD pool.
...
The DBOS MCP supports native integrations with custom agent networks. This allows you to build your agent logic in Python, Go, or TypeScript using official SDKs or raw SSE, while utilizing the DBOS web UI for rendering and state tracking.
