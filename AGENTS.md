...
- **High-Performance NVMe SSD Storage**: All Docker containers, event streams, and PostgreSQL databases (specifically the `kruschdb-postgres-data` volume) reside natively on the high-speed NVMe SSD (`/mnt/nvme/docker` on `kruschdev`) via system-level `"data-root"` configuration. This ensures ultra-low-latency database reads/writes and embedding queries.
