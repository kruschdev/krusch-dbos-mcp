...
import { OrchestrationCommandQueueLive } from "../src/persistence/Services/OrchestrationCommandQueue.ts";
...
import { makeTestPgPersistenceLive } from "../src/persistence/Layers/PostgresTest.ts";
...
function waitFor<A, E>(
  ...
  return read.pipe(
    ...
    Effect.mapError((error) => {
      if (error === RETRY_SIGNAL) {
        return new WaitForTimeoutError({ description });
      }
      return error;
    }),
    Effect.orDie,
...
export const makeOrchestrationIntegrationHarness = (
  ...
  Effect.gen(function* () {
    ...
    yield* Stream.runForEach(runtimeReceiptBus.streamEventsForTest, (receipt) => {
      return Ref.update(receiptHistory, (history) => [...history, receipt]).pipe(Effect.asVoid);
    ...
    const waitForThread: OrchestrationIntegrationHarness["waitForThread"] = (
      ...
      waitFor(
        snapshotQuery
          .getThreadDetailById(threadId as any)
          .pipe(Effect.map((threadOption) => Option.getOrNull(threadOption))),
    ...
    function waitForReceipt(
      ...
      const readMatchingReceipt = Ref.get(receiptHistory).pipe(
        Effect.tap((history) =>
          Effect.sync(() => {
            const matching = history.find(predicate);
            if (!matching) {
            } else {
            }
          }),
