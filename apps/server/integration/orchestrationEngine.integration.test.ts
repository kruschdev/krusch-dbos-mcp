...
const seedProjectAndThread = (harness: OrchestrationIntegrationHarness) =>
  Effect.gen(function* () {
    ...
    yield* harness.waitForThread(THREAD_ID, () => true);
  });
...
interface ItLive {
  (name: string, f: () => Effect.Effect<any, any, Scope.Scope>, timeout?: number): void;
  skipIf: (
    condition: boolean,
  ) => (name: string, f: () => Effect.Effect<any, any, Scope.Scope>, timeout?: number) => void;
  runIf: (
...
describe.sequential("OrchestrationEngine Integration Tests", () => {
  itLive("runs a single turn end-to-end and persists checkpoint state in sqlite + git", () =>
    withHarness((harness) =>
      Effect.gen(function* () {
  ...
  itLive.skipIf(!process.env.CODEX_BINARY_PATH)(
  ...
  itLive("runs multi-turn file edits and persists checkpoint diffs", () =>
    withHarness((harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
  ...
  itLive("tracks approval requests and resolves pending approvals on user response", () =>
    withHarness((harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
  ...
  itLive("records failed turn runtime state and checkpoint status as error", () =>
    withHarness((harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
  ...
  itLive("reverts to an earlier checkpoint and trims checkpoint projections + git refs", () =>
    withHarness((harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
  ...
  itLive.runIf(!!process.env.ANTHROPIC_API_KEY)(
    "starts a claudeAgent session on first turn when provider is requested",
    () =>
      withHarness(
        (harness) =>
          Effect.gen(function* () {
            ...
            const thread = yield* harness.waitForThread(
              ...
              (entry) =>
                ...
                entry.messages.some(
                  (message) =>
                    message.role === "assistant" && message.text === "Claude first turn.\n",
                ),
            );
  ...
  itLive.runIf(!!process.env.ANTHROPIC_API_KEY)(
    "forwards claudeAgent approval responses to the provider session",
    () =>
      withHarness(
        (harness) =>
          Effect.gen(function* () {
  ...
  itLive.runIf(!!process.env.ANTHROPIC_API_KEY)(
    "forwards thread.turn.interrupt to claudeAgent provider sessions",
    () =>
      withHarness(
        (harness) =>
          Effect.gen(function* () {
