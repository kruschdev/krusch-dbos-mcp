...
import { Effect, FileSystem, Layer, Path, Queue, Scope, Stream } from "effect";
...
interface ItLive {
  ...
  only: (name: string, f: () => Effect.Effect<any, any, Scope.Scope>, timeout?: number) => void;
...
itLive.only = (name: string, f: () => Effect.Effect<any, any, Scope.Scope>, timeout?: number) => {
  it.only(name, () => Effect.runPromise(Effect.scoped(f())), timeout);
};
...
itLive("replays typed runtime fixture events", () =>
  Effect.gen(function* () {
...
itLive("replays file-changing fixture turn events", () =>
  Effect.gen(function* () {
    const fixture = yield* makeIntegrationFixture;
...
itLive("runs multi-turn tool/approval flow", () =>
  Effect.gen(function* () {
    const fixture = yield* makeIntegrationFixture;
    const { join } = yield* Path.Path;
    const { writeFileString } = yield* FileSystem.FileSystem;
...
itLive("rolls back provider conversation state only", () =>
  Effect.gen(function* () {
    const fixture = yield* makeIntegrationFixture;
    const { join } = yield* Path.Path;
