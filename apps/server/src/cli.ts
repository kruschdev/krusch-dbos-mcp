...
import { parsePersistedServerObservabilitySettings } from "@kd/shared/serverSettings";
import {
  AuthSessionId,
  CommandId,
...
import { PostgresPersistenceLayerLive as PgPersistenceLayerLive } from "./persistence/Layers/Postgres.ts";
...
const BootstrapEnvelopeSchema = Schema.Struct({
  ...
  kdHome: Schema.optional(Schema.String),
...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to KD_HOME)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to KD_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("KD_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("KD_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("KD_TRACE_TIMING_ENABLED").pipe(Config.withDefault(true)),
  traceFile: Config.string("KD_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("KD_TRACE_MAX_BYTES").pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.int("KD_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("KD_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(200)),
  otlpTracesUrl: Config.string("KD_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("KD_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpExportIntervalMs: Config.int("KD_OTLP_EXPORT_INTERVAL_MS").pipe(
    Config.withDefault(10_000),
  ),
  otlpServiceName: Config.string("KD_OTLP_SERVICE_NAME").pipe(Config.withDefault("kd-server")),
  mode: Config.schema(RuntimeMode, "KD_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("KD_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("KD_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  kdHome: Config.string("KD_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("KD_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("KD_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("KD_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("KD_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  ollamaUrl: Config.string("OLLAMA_URL").pipe(
    Config.withDefault("http://localhost:11434/api/embeddings"),
  ),
});
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(
          ...
          Option.fromUndefinedOr(env.kdHome),
          Option.fromUndefinedOr(bootstrap?.kdHome),
        ),
      ),
    );
    ...
    const config: ServerConfigShape = {
      ...
      geminiApiKey: env.geminiApiKey ?? bootstrap?.geminiApiKey ?? undefined,
      ollamaUrl: env.ollamaUrl,
      otlpExportIntervalMs: env.otlpExportIntervalMs ?? 10_000,
...
const ProjectCliRuntimeLive = Layer.mergeAll(
  ...
  OrchestrationLayerLive.pipe(
    ...
    Layer.provideMerge(PgPersistenceLayerLive),
  ),
);
...
const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the Krusch DBOS server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
