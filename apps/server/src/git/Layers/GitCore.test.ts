...
import { GitCommandError } from "@kd/contracts";
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "kdcode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "kdcode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "kdcode/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "kdcode/tmp-working",
          newBranch: "kdcode/feat/session",
        });

        expect(renamed.branch).toBe("kdcode/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "kdcode/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "kdcode/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("kdcode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "kdcode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "kdcode/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "kdcode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "kdcode/tmp-working" });
        ...
        expect(renamed.branch).toBe("kdcode/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/kdcode.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/kdcode.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "kdcode/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
