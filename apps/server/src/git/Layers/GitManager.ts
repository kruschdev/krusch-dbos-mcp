...
import { GitManagerError } from "@kd/contracts";
import {
  GitManager,
...
import type { GitManagerServiceError } from "@kd/contracts";
import {
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `kdcode/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `kdcode-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
