...
const makeRoutingTextGeneration = Effect.gen(function* () {
  const codex = yield* Effect.service(CodexTextGen);
  const claude = yield* Effect.service(ClaudeTextGen);
  const cursor = yield* Effect.service(CursorTextGen);
  const openCode = yield* Effect.service(OpenCodeTextGen);
...
const InternalCodexLayer = Layer.effect(
  CodexTextGen,
  Effect.gen(function* () {
    const svc = yield* Effect.service(TextGeneration);
    return svc;
  }),
