    ...
    it.effect("stores message attachment references without mutating payloads", () =>
      Effect.gen(function* () {
        ...
        const rows = yield* sql<{
          readonly attachmentsJson: unknown;
        }>`
            SELECT
        ...
        assert.deepEqual(rows[0]?.attachmentsJson ?? null, [
          {
            type: "image",
            id: "thread-attachments-att-1",
            name: "example.png",
...
it.layer(
  ...
  it.effect("does not persist attachment files when projector transaction rolls back", () =>
    Effect.gen(function* () {
      ...
      yield* sql`
        CREATE OR REPLACE FUNCTION fail_thread_messages_projection_state_update_fn()
        RETURNS TRIGGER AS '
        BEGIN
          IF NEW.projector = ''projection.thread-messages'' THEN
            RAISE EXCEPTION ''forced-projection-state-failure'';
          END IF;
          RETURN NEW;
        END;
        ' LANGUAGE plpgsql;
      ...
      yield* sql`
        ...
        FOR EACH ROW
        EXECUTE FUNCTION fail_thread_messages_projection_state_update_fn();
      `;
...
describe("OrchestrationProjectionPipeline via engine dispatch", () => {
  ...
  itLive("projects dispatched engine events immediately", () =>
    Effect.gen(function* () {
      ...
      const projectRows = yield* sql<{ readonly title: string; readonly scriptsJson: unknown }>`
        SELECT
          title,
          scripts_json AS "scriptsJson"
        FROM projection_projects
  ...
  itLive("projects persist updated scripts from project.meta.update", () =>
    Effect.gen(function* () {
      ...
      const projectRows = yield* sql<{
        readonly scriptsJson: any;
        readonly defaultModelSelection: any;
      }>`
        SELECT
          scripts_json AS "scriptsJson",
