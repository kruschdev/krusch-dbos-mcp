...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listThreadMessageRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        SELECT
          ptm.message_id AS "messageId",
          ptm.thread_id AS "threadId",
          ptm.turn_id AS "turnId",
          ptm.role,
          ptm.text,
          ptm.attachments_json AS "attachments",
          ptm.is_streaming AS "isStreaming",
          ptm.created_at AS "createdAt",
          ptm.updated_at AS "updatedAt"
        FROM projection_thread_messages ptm
        JOIN projection_threads pt ON ptm.thread_id = pt.thread_id
        WHERE pt.deleted_at IS NULL
        ORDER BY ptm.thread_id ASC, ptm.created_at ASC, ptm.message_id ASC
      `,
  });
  ...
  const listThreadProposedPlanRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        SELECT
          ptpp.plan_id AS "planId",
          ptpp.thread_id AS "threadId",
          ptpp.turn_id AS "turnId",
          ptpp.plan_markdown AS "planMarkdown",
          ptpp.implemented_at AS "implementedAt",
          ptpp.implementation_thread_id AS "implementationThreadId",
          ptpp.created_at AS "createdAt",
          ptpp.updated_at AS "updatedAt"
        FROM projection_thread_proposed_plans ptpp
        JOIN projection_threads pt ON ptpp.thread_id = pt.thread_id
        WHERE pt.deleted_at IS NULL
        ORDER BY ptpp.thread_id ASC, ptpp.created_at ASC, ptpp.plan_id ASC
      `,
  });
  ...
  const listThreadActivityRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        SELECT
          pta.activity_id AS "activityId",
          pta.thread_id AS "threadId",
          pta.turn_id AS "turnId",
          pta.tone,
          pta.kind,
          pta.summary,
          pta.payload_json AS "payload",
          pta.sequence,
          pta.created_at AS "createdAt"
        FROM projection_thread_activities pta
        JOIN projection_threads pt ON pta.thread_id = pt.thread_id
        WHERE pt.deleted_at IS NULL
        ORDER BY
          pta.thread_id ASC,
          CASE WHEN pta.sequence IS NULL THEN 0 ELSE 1 END ASC,
          pta.sequence ASC,
          pta.created_at ASC,
          pta.activity_id ASC
      `,
  ...
  const listThreadSessionRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        SELECT
          pts.thread_id AS "threadId",
          pts.status,
          pts.provider_name AS "providerName",
          pts.provider_session_id AS "providerSessionId",
          pts.provider_thread_id AS "providerThreadId",
          pts.runtime_mode AS "runtimeMode",
          pts.active_turn_id AS "activeTurnId",
          pts.last_error AS "lastError",
          pts.updated_at AS "updatedAt"
        FROM projection_thread_sessions pts
        JOIN projection_threads pt ON pts.thread_id = pt.thread_id
        WHERE pt.deleted_at IS NULL
        ORDER BY pts.thread_id ASC
      `,
  });
  ...
  const listCheckpointRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        SELECT
          pturn.thread_id AS "threadId",
          pturn.turn_id AS "turnId",
          pturn.checkpoint_turn_count AS "checkpointTurnCount",
          pturn.checkpoint_ref AS "checkpointRef",
          pturn.checkpoint_status AS "status",
          pturn.checkpoint_files_json AS "files",
          pturn.assistant_message_id AS "assistantMessageId",
          pturn.completed_at AS "completedAt"
        FROM projection_turns pturn
        JOIN projection_threads pt ON pturn.thread_id = pt.thread_id
        WHERE pturn.checkpoint_turn_count IS NOT NULL
          AND pt.deleted_at IS NULL
        ORDER BY pturn.thread_id ASC, pturn.checkpoint_turn_count ASC
      `,
  });
    ...
    execute: () =>
      sql`
        SELECT
          ...
          pturn.turn_id AS "turnId",
          pturn.state,
          pturn.requested_at AS "requestedAt",
          pturn.started_at AS "startedAt",
          pturn.completed_at AS "completedAt",
          pturn.assistant_message_id AS "assistantMessageId",
          pturn.source_proposed_plan_thread_id AS "sourceProposedPlanThreadId",
          pturn.source_proposed_plan_id AS "sourceProposedPlanId"
        FROM projection_turns pturn
        JOIN projection_threads pt ON pturn.thread_id = pt.thread_id
        WHERE pturn.turn_id IS NOT NULL
          AND pt.deleted_at IS NULL
        ORDER BY pturn.thread_id ASC, pturn.requested_at DESC, pturn.turn_id DESC
      `,
  });
  ...
  const readProjectionCounts = SqlSchema.findOne({
    ...
    execute: () =>
      sql`
        SELECT
          (SELECT COUNT(*)::INTEGER FROM projection_projects) AS "projectCount",
          (SELECT COUNT(*)::INTEGER FROM projection_threads) AS "threadCount"
      `,
  });
