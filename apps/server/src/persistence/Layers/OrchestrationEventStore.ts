...
const EventMetadataJson = OrchestrationEventMetadata;
...
const AppendEventRequestSchema = Schema.Struct({
  ...
  payloadJson: UnknownJson,
  metadataJson: EventMetadataJson,
});
...
const makeEventStore = Effect.gen(function* () {
  ...
  const append: OrchestrationEventStoreShape["append"] = (event) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          yield* sql`SELECT pg_advisory_xact_lock(hashtext(${event.aggregateKind}), hashtext(${event.aggregateId}))`;

          return yield* appendEventRow({
