...
import { parseCliArgs } from "@kd/shared/cliArgs";
import {
  ApprovalRequestId,
...
import { applyClaudePromptEffortPrefix, resolveEffort, trimOrNull } from "@kd/shared/model";
import {
  Cause,
  DateTime,
  Deferred,
...
const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (originalInput) {
    let input = { ...originalInput };
    let promptText = input.input || "";
    if (promptText) {
      const ragContext = yield* fetchRAGContext(promptText, input.threadId, embeddingProviderSvc, sqlSvc);
    ...
    if (context.turnState?.interrupted) {
      const stamp = yield* makeEventStamp();
      yield* offerRuntimeEvent({
        type: "turn.aborted",
        eventId: stamp.eventId,
      ...
      yield* completeTurn(context, "cancelled");
    } else {
  ...
  const interruptTurn: ClaudeAdapterShape["interruptTurn"] = Effect.fn("interruptTurn")(
    function* (threadId, turnId) {
      ...
      if (context.turnState && (!turnId || context.turnState.turnId === turnId)) {
        context.turnState.interrupted = true;
      }
      yield* Effect.tryPromise({
