      ...
      probe: {
        ...
        message:
          openCodeSettings.serverUrl.trim().length > 0
            ? "OpenCode is disabled in Krusch DBOS settings. A server URL is configured."
            : "OpenCode is disabled in Krusch DBOS settings.",
      },
    });
  }
...
export const OpenCodeProviderLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const openCodeRuntime = yield* Effect.service(OpenCodeRuntime);
    ...
    const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (input: {
      ...
      if (!isExternalServer) {
        ...
        if (!version) {
          return fallback(
            new Error(
              `Unable to determine OpenCode version from \`opencode --version\` output. Krusch DBOS requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
            ),
            null,
          );
        }
