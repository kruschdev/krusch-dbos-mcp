...
const ProviderRegistryLiveBase = Layer.effect(
  ProviderRegistry,
  Effect.gen(function* () {
    const codexProvider = yield* Effect.service(CodexProvider);
    const claudeProvider = yield* Effect.service(ClaudeProvider);
    const openCodeProvider = yield* Effect.service(OpenCodeProvider);
    const config = yield* Effect.service(ServerConfig);
    const fileSystem = yield* Effect.service(FileSystem.FileSystem);
    const path = yield* Effect.service(Path.Path);

    const cursorProvider = yield* Effect.service(CursorProvider);
    const geminiProvider = yield* Effect.service(GeminiProvider);
