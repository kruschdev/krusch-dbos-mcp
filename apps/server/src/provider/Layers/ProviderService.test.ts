...
import { makeTestPgPersistenceLive } from "../../persistence/Layers/PostgresTest.ts";
import { ServerSettingsService } from "../../serverSettings.ts";
import { AnalyticsService } from "../../telemetry/Services/AnalyticsService.ts";
import { ServerConfig } from "../../config.ts";

const defaultServerSettingsLayer = ServerSettingsService.layerTest();
const defaultServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "t3-provider-test-",
});
...
function makeProviderServiceLayer() {
  ...
  const layer = it.layer(
    makeProviderServiceLive().pipe(
      Layer.provideMerge(providerAdapterLayer),
      Layer.provideMerge(directoryLayer),
      Layer.provideMerge(defaultServerSettingsLayer),
      Layer.provideMerge(defaultServerConfigLayer),
...
it.effect("ProviderServiceLive rejects new sessions for disabled providers", () =>
  Effect.gen(function* () {
    ...
    assert.include(failure.issue, "Provider 'claudeAgent' is disabled in Krusch DBOS settings.");
...
it.effect("ProviderServiceLive writes canonical events to the emitting thread segment", () =>
  Effect.gen(function* () {
    ...
    const providerLayer = makeProviderServiceLive({
      ...
      Layer.provideMerge(Layer.succeed(ProviderAdapterRegistry, registry)),
      Layer.provideMerge(directoryLayer),
      Layer.provideMerge(defaultServerSettingsLayer),
      Layer.provideMerge(defaultServerConfigLayer),
      Layer.provideMerge(AnalyticsService.layerTest),
...
it.effect("ProviderServiceLive keeps persisted resumable sessions on startup", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      yield* sql`TRUNCATE TABLE provider_session_runtime CASCADE`;
    ...
    // SQLite assertion removed because architecture migrated to PostgreSQL
  }).pipe(Effect.provide(NodeServices.layer)),
);
...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("reuses persisted resume cursor when startSession is called after a restart", () =>
    Effect.gen(function* () {
      ...
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "t3-provider-claude-start-"));
      ...
      if (resumedStartInput && typeof resumedStartInput === "object") {
        ...
        assert.equal(startPayload.provider, "claudeAgent");
        assert.equal(startPayload.cwd, tempDir);
        assert.deepEqual(startPayload.resumeCursor, initial.resumeCursor);
        assert.equal(startPayload.threadId, initial.threadId);
      }
