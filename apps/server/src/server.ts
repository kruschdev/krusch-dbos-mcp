...
import { mcpSseRouteLayer, mcpMessagesRouteLayer } from "./mcpServer.ts";
import { OpenLive } from "./open.ts";
import { PostgresPersistenceLayerLive } from "./persistence/Layers/Postgres.ts";
...
import { VectorEmbeddingWorkerLive } from "./orchestration/Layers/VectorEmbeddingWorker.ts";
...
import { EmbeddingProviderLive } from "./orchestration/Layers/EmbeddingProvider.ts";

const ReactorLayerLive = ServerSettingsLive.pipe(
  ...
  Layer.provideMerge(VectorEmbeddingWorkerLive),
  Layer.provideMerge(EmbeddingProviderLive),
);
...
const ProviderLayerLive = Layer.unwrap(
  Effect.gen(function* () {
    ...
    ).pipe(Layer.provide(EmbeddingProviderLive));
...
const PersistenceLayerLive = Layer.empty.pipe(Layer.provideMerge(PostgresPersistenceLayerLive));
...
export const runServer = Layer.launch(makeServerLayer) as any as Effect.Effect<
  never,
  any,
  ServerConfig
>;
