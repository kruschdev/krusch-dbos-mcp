...
import { type FilesystemBrowseInput, type ProjectEntry } from "@kd/contracts";
import { isExplicitRelativePath, isWindowsAbsolutePath } from "@kd/shared/path";
import {
  insertRankedSearchResult,
  normalizeSearchQuery,
  scoreQueryMatch,
  type RankedSearchResult,
} from "@kd/shared/searchRanking";
    ...
    function* (input) {
      ...
      const entries = yield* Effect.tryPromise({
        try: async () => {
          ...
          for (const dirent of dirents) {
            if (!dirent.name.toLowerCase().startsWith(lowerPrefix)) continue;
            if (!showHidden && dirent.name.startsWith(".")) continue;
            
            let isDir = dirent.isDirectory();
            if (!isDir && dirent.isSymbolicLink()) {
              try {
                const stat = await fsPromises.stat(path.join(parentPath, dirent.name));
