...
export const EmbeddingProviderKind = Schema.Literals(["none", "ollama", "gemini", "openai"]);
export type EmbeddingProviderKind = typeof EmbeddingProviderKind.Type;

export const EmbeddingSelection = Schema.Struct({
...
export type EmbeddingSelection = typeof EmbeddingSelection.Type;
...
export const ServerSettings = Schema.Struct({
  ...
  embeddingModelSelection: EmbeddingSelection.pipe(
    Schema.withDecodingDefault(
      Effect.succeed({
  ...
  providers: Schema.Struct({
    codex: CodexSettings.pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
    claudeAgent: ClaudeSettings.pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
    cursor: CursorSettings.pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
    opencode: OpenCodeSettings.pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
    gemini: GeminiSettings.pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
  }).pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
  observability: ObservabilitySettings.pipe(Schema.withDecodingDefault(Effect.succeed({} as any))),
});
...
const GeminiModelOptionsPatch = Schema.Struct({
  thinking: Schema.optionalKey(GeminiModelOptions.fields.thinking),
  fastMode: Schema.optionalKey(GeminiModelOptions.fields.fastMode),
  contextWindow: Schema.optionalKey(GeminiModelOptions.fields.contextWindow),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  embeddingModelSelection: Schema.optionalKey(
    Schema.Struct({
      provider: Schema.optionalKey(EmbeddingProviderKind),
