...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit KD_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "KD_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid KD_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults KD_HOME to ~/.kd when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KD_HOME, path.resolve(NodeOS.homedir(), ".kd"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KD_HOME, path.resolve("/tmp/custom-t3"));
        assert.equal(env.KD_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://127.0.0.1:4222");
        assert.equal(env.VITE_WS_URL, "ws://127.0.0.1:4222");
        assert.equal(env.KD_NO_BROWSER, "1");
        assert.equal(env.KD_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.KD_LOG_WS_EVENTS, "1");
        assert.equal(env.KD_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KD_MODE, "web");
        assert.equal(env.KD_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KD_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom kdHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.KD_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KD_PORT, "13773");
