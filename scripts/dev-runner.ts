...
export const DEFAULT_KD_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".kd"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("KD_PORT_OFFSET"),
  devInstance: optionalStringConfig("KD_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid KD_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `KD_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric KD_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed KD_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly kdHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(kdHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      KD_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.KD_PORT = String(serverPort);
      ...
      delete output.KD_MODE;
      delete output.KD_NO_BROWSER;
      delete output.KD_HOST;
    }
    ...
    if (!isDesktopMode && noBrowser !== undefined) {
      output.KD_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.KD_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.KD_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.KD_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.KD_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.KD_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.KD_MODE = "web";
      delete output.KD_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read KD_PORT_OFFSET/KD_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KD_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.KD_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  kdHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Krusch DBOS data (equivalent to KD_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("KD_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to KD_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("KD_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to KD_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("KD_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to KD_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("KD_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to KD_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("KD_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to KD_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("KD_PORT")),
  ),
